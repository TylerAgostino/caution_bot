    ...
    configured_events = [
        {
            ...
            "args": {
                key.split(f"{i}")[1]: st.session_state[key]
                for key in st.session_state.keys()
    ...
    if "event_classes" in st.session_state:
        for guid, event in st.session_state["event_classes"].items():
            properties = {
                ...
                for key in st.session_state.keys()
                if key.startswith(guid) and key != f"{guid}_type"
            }
...
def save_preset(name):
    ...
    file.write(
        json.dumps(
            st.session_state.get("configured_events", []), indent=4, sort_keys=True
        )
    )
    file.close()
...
def start():
    st.session_state.event_classes = {
        event["guid"]: event_types[event["type"]](**event["args"])
        for event in st.session_state.get("configured_events", [])
    }
    event_run_methods = [
...
def ui():
    ...
    for i in st.session_state["events"]:
        ...
        with left:
            ...
            st.selectbox(
                f"Select Event Type for Event {i}",
                list(event_types.keys()),
                key=f"{i}_type",
                on_change=onchange,
                args=[i],
                index=list(event_types.keys()).index(
                    st.session_state.get(f"{i}_type", "Lap Caution Event")
                ),
            )
