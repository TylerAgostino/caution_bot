...
class RandomTimedEvent(RandomEvent):
    ...
    def is_time_to_start(self, adjustment=0):
        ...
        time_until_trigger = (
            self.start_time - (total_session_time - time_remaining) + adjustment
        )
    ...
    def check_and_set_quickie_flag(self):
        ...
        if (
            (
                total_session_time - time_remaining
                >= self.start_time - self.quickie_window * 60
            )
            and (
                self.is_caution_active()
                or self.busy_event.is_set()
                or (self.start_time <= self.quickie_window * 60)
            )
