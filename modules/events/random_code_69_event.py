...
class RestartOrderManager:
    def __init__(self, sdk, preset_order=None):
        ...
        self.one_meter = 1 / (
            float(str(self.sdk["WeekendInfo"]["TrackLength"]).replace(" km", "")) * 1000
        )
        ...
        self.class_speed_rank = {
            class_: str(i + 1)
            for i, class_ in enumerate(
                sorted(
                    {k: v for k, v in self.class_lap_times.items() if v is not None},
                    key=lambda x: self.class_lap_times[x],
                )
            )
        }
    ...
    def add_car_to_order(self, carIdx, wave_around=0, slower_class_catchup=0):
        ...
        if carIdx not in [car["CarIdx"] for car in self.order]:
            ...
            car_restart_record = [car for car in self.order if car["CarIdx"] == carIdx][
                0
            ]
            self.order.remove(car_restart_record)
            car_restart_record["BeganPacingTick"] = int(self.sdk["SessionTick"])
            car_restart_record["LatePit"] = (
                1  # if car_restart_record['ActualPosition'] >= 0.25 else 0
            )
        ...
        if self.class_separation:
            self.order = sorted(
                ...
                key=lambda x: (
                    x["LatePit"],
                    x["CarClassOrder"],
                    x["WaveAround"] + x["SlowerClassCatchup"],
                    x["BeganPacingTick"],
                    -x["BeganPacingDistance"],
                ),
            )
        else:
            self.order = sorted(
    ...
    def update_car_positions(self):
        ...
        for i, car in enumerate(self.order):
            ...
            for car_ahead in self.order[:i]:
                wave_adjusted_car_ahead_position = (
                    car_ahead["ActualPosition"]
                    - car_ahead["WaveAround"]
                    - car_ahead["SlowerClassCatchup"]
                    + car["WaveAround"]
                    + car["SlowerClassCatchup"]
                )
                if (
                    car["ActualPosition"] > wave_adjusted_car_ahead_position
                    and not self.sdk["CarIdxOnPitRoad"][car_ahead["CarIdx"]]
                    and not self.sdk["CarIdxOnPitRoad"][car["CarIdx"]]
                    and not self.sdk["CarIdxLapDistPct"][car_ahead["CarIdx"]] == -1
                    and not self.sdk["CarIdxLapDistPct"][car["CarIdx"]] == -1
                ):
                    self.order[i]["IncorrectOvertakes"].append(car_ahead["CarNumber"])
            for car_behind in self.order[i + 1 :]:
                wave_adjusted_car_behind_position = (
                    car_behind["ActualPosition"]
                    - car_behind["WaveAround"]
                    - car_behind["SlowerClassCatchup"]
                    + car["WaveAround"]
                    + car["SlowerClassCatchup"]
                )
                if (
                    car["ActualPosition"] < wave_adjusted_car_behind_position
                    and not self.sdk["CarIdxOnPitRoad"][car_behind["CarIdx"]]
                    and not self.sdk["CarIdxOnPitRoad"][car["CarIdx"]]
                    and not self.sdk["CarIdxLapDistPct"][car_behind["CarIdx"]] == -1
                    and not self.sdk["CarIdxLapDistPct"][car["CarIdx"]] == -1
                ):
                    self.order[i]["IncorrectlyOvertakenBy"].append(
                        car_behind["CarNumber"]
                    )
...
class RandomTimedCode69Event(RandomTimedEvent):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.quickie_auto_restart_get_ready_position = (
            quickie_auto_restart_get_ready_position
        )
        self.quickie_auto_restart_form_lanes_position = (
        ...
        self.pacing_speed_km = lambda: (
            self.wet_speed_km if self.sdk["WeatherDeclaredWet"] else self.max_speed_km
        )
    ...
    def ui(ident=""):
        ...
        return {
            "min": col1.number_input(
                f"Window Start",
                value=5,
                key=f"{ident}min",
                help="Minutes/Laps before the start of the window. Negative values are relative to the end of the race.",
            ),
            "max": col1.number_input(
                "Window End",
                value=-15,
                key=f"{ident}max",
                help="Minutes/Laps before the end of the window. Negative values are relative to the end of the race",
            ),
            "reminder_frequency": aa_col1.number_input(
                "Reminder Frequency",
                value=8,
                help="How often to send reminders in chat. If this is too low, the bot may spam the chat and be unresponsive.",
                key=f"{ident}reminder_frequency",
            ),
            "likelihood": col2.number_input(
                f"% Chance",
                key=f"{ident}likelihood",
                value=75,
                help="The likelihood of the event happening. 100% means it will happen every time.",
            ),
            "max_speed_km": aa_col2.number_input(
                "Pace Speed (kph)",
                value=69,
                help="Pesters the leader to stay below this speed.",
                key=f"{ident}max_speed_km",
            ),
            "wet_speed_km": aa_col3.number_input(
                "Wet Pace Speed (kph)",
                value=69,
                help="Pesters the leader to stay below this speed when the track is wet.",
                key=f"{ident}wet_speed_km",
            ),
            "auto_class_separate_position": col3.number_input(
                "Class Separation Position",
                value=-1.00,
                help="Laps of pacing before separating classes. -1 to disable auto class separation",
                key=f"{ident}auto_class_separate_position",
            ),
            "wave_arounds": col3.checkbox(
                f"Wave Arounds", key=f"{ident}wave_arounds", value=True
            ),
            "auto_restart_form_lanes_position": col4.number_input(
                "Lanes Form Position",
                value=1.63,
                help="Laps of pacing before forming the restart lanes. -1 to disable auto lane forming",
                key=f"{ident}auto_restart_form_lanes_position",
            ),
            "lane_names": col4.text_input(
                "Lane Names",
                "Right,Left",
                help="A comma-separated list of lane names. Length must be equal to the number of restart lanes. Primary/Lead lane is the first in the list.",
                key=f"{ident}lane_names",
            ).split(","),
            "auto_restart_get_ready_position": col5.number_input(
                "Restart Position",
                value=1.79,
                help="Laps of pacing before restarting. -1 to disable auto restart",
                key=f"{ident}auto_restart_get_ready_position",
            ),
            "restart_speed_pct": aa_col2.number_input(
                "Restart Speed %",
                value=125,
                help='After the leader receives the "You control the field" message, show the green flag when they reach this % of the pacing speed',
                key=f"{ident}restart_speed_pct",
            ),
            # quickie stuff
            "quickie_window": aa_col1.number_input(
                "Quickie Window",
                value=5,
                help="If within this many minutes of another event, make this a quickie 69. -1 to disable",
                key=f"{ident}quickie_window",
            ),
            "quickie_auto_class_separate_position": aa_col3.number_input(
                "Quickie Class Separation Position",
                value=-1,
                help="Laps of pacing before separating classes (during Quickie 69)",
                key=f"{ident}quickie_auto_class_separate_position",
            ),
            "quickie_auto_restart_form_lanes_position": aa_col3.number_input(
                "Quickie Lanes Form Position",
                value=0.63,
                help="Laps of pacing before forming the restart lanes (during Quickie 69)",
                key=f"{ident}quickie_auto_restart_form_lanes_position",
            ),
            "quickie_auto_restart_get_ready_position": aa_col3.number_input(
                "Quickie Restart Position",
                value=0.79,
                help="Laps of pacing before restarting (during Quickie 69)",
                key=f"{ident}quickie_auto_restart_get_ready_position",
            ),
            "quickie_invert_lanes": aa_col1.checkbox(
                f"Invert Quickie Lanes",
                key=f"{ident}quickie_invert_lanes",
                value=False,
                help="Inverts the lane names for the quickie event.",
            ),
            "notify_on_skipped_caution": aa_col1.checkbox(
                f"Notify on Skip",
                key=f"{ident}notify_on_skipped_caution",
                value=False,
                help="Send a message to the chat if the event is triggered and skipped while another event is in progress.",
            ),
            "end_of_lap_safety_margin": aa_col2.number_input(
                "End of Lap Safety Margin",
                value=0.1,
                help="If the leader is within this lap percent of the S/F when the code69 is triggered, extend by another lap",
                key=f"{ident}end_of_lap_safety_margin",
            ),
        }
    ...
    def send_reminders(self, order_generator):
        ...
        for car in order_generator.wave_around_cars:
            if self.wave_arounds_active:
                self._chat(
                    f"/{car['CarNumber']} Safely overtake the leader and join at the back of the pack."
                )
        for car in order_generator.out_of_place_cars:
            self._chat(
                f"/{car['CarNumber']} Let the {', '.join(car['IncorrectOvertakes'])} car{'s' if len(car['IncorrectOvertakes']) > 1 else ''} by."
            )
        for car in order_generator.displaced_cars:
            self._chat(
                f"/{car['CarNumber']} Pass the {', '.join(car['IncorrectlyOvertakenBy'])} car{'s' if len(car['IncorrectlyOvertakenBy']) > 1 else ''}."
            )
    ...
    def event_sequence(self):
        ...
        if self.is_caution_active() or self.busy_event.is_set():
            if self.notify_on_skipped_caution:
                self._chat(
                    "Additional caution skipped due to active caution.",
                    race_control=True,
                )
            return
        ...
        send_message_indicator = self.intermittent_boolean_generator(
        ...
        while not any([car["LapCompleted"] > lead_lap for car in this_step]):
            ...
            for car in this_step:
                if (
                    self.car_has_completed_lap(car, last_step, this_step)
                    and not self.sdk["CarIdxOnPitRoad"][car["CarIdx"]]
                ) or (
                    self.car_has_left_pits(car, last_step, this_step)
                    and self.sdk["CarIdxLapDistPct"][car["CarIdx"]] < 0.5
                ):
                    self._chat(
                        f"/{car['CarNumber']} YOU ARE STILL RACING FOR AN ADDITIONAL LAP"
                    )
        ...
        while not self.restart_ready.is_set():
            ...
            for class_ in classes:
                cars_in_class_on_track = [
                    car
                    for car in this_step
                    if self.get_car_class(carIdx=car["CarIdx"]) == class_
                    and not car["InPits"]
                ]
                cars_in_class_in_pits = [
                    car
                    for car in this_step
                    if self.get_car_class(carIdx=car["CarIdx"]) == class_
                    and car["InPits"]
                ...
                if cars_in_class_on_track:
                    ...
                    leader_in_pits = (
                        cars_in_class_in_pits[0] if cars_in_class_in_pits else None
                    )
                    if (
                        leader_in_pits
                        and leader_in_pits["LapCompleted"]
                        > leader_on_track["LapCompleted"]
                    ):
            ...
            for car in this_step:
                existing_restart_record = [
                    ...
                    if c["CarIdx"] == car["CarIdx"]
                ]
                car_flags = self.sdk["CarIdxSessionFlags"][car["CarIdx"]]
                if (
                    existing_restart_record
                    and self.car_has_entered_pits(car, last_step, this_step)
                    and existing_restart_record[0]["ActualPosition"] < 0.5
                ):
                    # welcome to Bathurst
                    self.logger.debug(
                        f"Car {car['CarNumber']} has entered pits immediately after S/F, removing from order"
                    )
                    restart_order_generator.order = [
                        c
                        for c in restart_order_generator.order
                        if c["CarIdx"] != car["CarIdx"]
                    ]
                ...
                if (
                    (
                        ...
                        and self.car_has_completed_lap(car, last_step, this_step)
                        and not self.sdk["CarIdxOnPitRoad"][car["CarIdx"]]
                    )
                    or (
                        self.car_has_left_pits(car, last_step, this_step)
                        and [x for x in this_step if x["CarIdx"] == car["CarIdx"]][0][
                            "LapDistPct"
                        ]
                        < 0.5
                    )
                    or (
                        car["CarIdx"]
                        in [c["CarIdx"] for c in restart_order_generator.order]
                        and (
                            self.car_has_entered_pits(car, last_step, this_step)
                            or car_flags & self.Flags.black
                        )
                    )
                ):
                    try:
                        ...
                        if (
                            car["CarIdx"]
                            in [c["CarIdx"] for c in restart_order_generator.order]
                            and car_flags & self.Flags.black
                        ):
                            self.logger.debug(
                                f"Car {car['CarNumber']} has a black flag, converting to EOL"
                            )
                    ...
                    if gets_wave_around:
                        ...
                        self.logger.debug(
                            f"Class Leader Laps Completed: {class_leader_distance_completed}"
                        )

                        self.logger.debug(
                            f"Class Leader in pits: {class_leader_in_pits}"
                        )

                    restart_order_generator.add_car_to_order(
                        car["CarIdx"],
                    ...
                    if (
                        ...
                        self._chat(
                            f"/{car['CarNumber']} Catch the field and look for further instructions"
                        )
                    else:
                        self._chat(
                            f"/{car['CarNumber']} Slow down and look for further instructions"
                        )
                    self.logger.debug(
                        f"Adding car {car['CarNumber']} to order (completed lap)"
                    )
                    self.logger.debug(
                        f"Wave Around: {gets_wave_around}, Catch Up: {gets_catch_up}"
                    )
                    self.logger.debug(f"Correct order: {restart_order_generator.order}")
                    if gets_wave_around or gets_catch_up:
                        self.logger.info(
                            f"{car['CarNumber']} gets wave around: {gets_wave_around}, catch up: {gets_catch_up}"
                        )
                    continue
            self.sdk.unfreeze_var_buffer_latest()
            self.sdk.freeze_var_buffer_latest()
            ...
            if send_message_indicator.__next__():
                ...
                if len(correct_order) > 0 and (
                    leader_speed_generator is None
                    or leader["CarNumber"]
                    != restart_order_generator.leader()["CarNumber"]
                    or leader["CarNumber"]
                    not in [car["CarNumber"] for car in restart_order_generator.order]
                ):
            ...
            if (
                ...
                or (
                    0
                    < (
                        self.quickie_auto_class_separate_position
                        if self.quickie
                        else self.auto_class_separate_position
                    )
                ...
                [
                    i
                    for i, v in restart_order_generator.class_lap_times.items()
                    if v is not None
                ]
            ) > 1:
            ...
            if (
                ...
                < (
                    self.quickie_auto_restart_form_lanes_position
                    if self.quickie
                    else self.auto_restart_form_lanes_position
                )
                <= restart_order_generator.leader()["ActualPosition"]
                and restart_order_generator.order
            ):
                ...
                < (
                    self.quickie_auto_restart_get_ready_position
                    if self.quickie
                    else self.auto_restart_get_ready_position
                )
                <= restart_order_generator.leader()["ActualPosition"]
                and restart_order_generator.order
            ):
        ...
        lane_names = (
            lane_names[::-1]
            if self.quickie and self.quickie_invert_lanes
            else lane_names
        )
        this_step = self.get_current_running_order()
        ...
        if self.extra_lanes:
            ...
            for car in restart_order_generator.order:
                ...
                self._chat(
                    f"/{car['CarNumber']} Line up {number_of_lanes} wide in the {str(lane_names[i % number_of_lanes]).upper()} lane."
                )
                try:
            ...
            for lane_cars in lanes_raw:
                lane_order_generators.append(
                    RestartOrderManager(self.sdk, preset_order=lane_cars)
                )
        ...
        less_frequent_messages = self.intermittent_boolean_generator(
            self.reminder_frequency * 2
        )
        while not self.restart_ready.is_set():
            self.sdk.unfreeze_var_buffer_latest()
            self.sdk.freeze_var_buffer_latest()
            ...
            for i in range(number_of_lanes):
                ...
                for car in this_step:
                    if (
                        self.car_has_left_pits(car, last_step, this_step)
                        or self.car_has_entered_pits(car, last_step, this_step)
                    ) and car["CarIdx"] in [
                        lane["CarIdx"] for lane in lane_order_generators[i].order
                    ]:
                        lane_order_generators[i].add_car_to_order(car["CarIdx"])

            if lane_order_generators[0].leader()["ActualPosition"] >= (
                self.quickie_auto_restart_get_ready_position
                if self.quickie
                else self.auto_restart_get_ready_position
            ):
            ...
            if send_message_indicator.__next__():
                if len(lane_order_generators[0].order) > 0 and (
                    leader_speed_generator is None
                    or leader["CarNumber"]
                    != lane_order_generators[0].leader()["CarNumber"]
                    or leader["CarNumber"]
                    not in [car["CarNumber"] for car in lane_order_generators[0].order]
                ):
                    # New leader, we need a new speed generator
                ...
                for i in range(number_of_lanes):
                    ...
                    if lane_order_generators[i].leader() is None:
                        continue
                    if (
                        lane_order_generators[i].leader()["ActualPosition"]
                        > lane_order_generators[0].leader()["ActualPosition"]
                    ):
                        self._chat(
                            f"/{lane_order_generators[i].leader()['CarNumber']} DO NOT PASS THE {lane_order_generators[0].leader()['CarNumber']} CAR."
                        )
                if speed_km_per_hour > self.pacing_speed_km():
                    self._chat(
                        f"/{leader['CarNumber']} Slow down to {self.pacing_speed_km()} kph / {int(self.pacing_speed_km() * 0.621371)} mph."
            ...
            if less_frequent_messages.__next__():
                for i in range(number_of_lanes):
                    for car in lane_order_generators[i].order:
                        self._chat(
                            f"/{car['CarNumber']} Line up {number_of_lanes} wide in the {str(lane_names[i]).upper()} lane."
                        )
            self.sdk.unfreeze_var_buffer_latest()
            self.sdk.freeze_var_buffer_latest()
        ...
        self.sleep(2)
        throwaway_speed = (
            leader_speed_generator.__next__()
        )  # Make sure we aren't using an average from a while ago
        ...
        if immediate_throw:
            self._chat(
                f"/{leader['CarNumber']} {violation_message} - Accelerated too early"
            )
        ...
        for i in range(len(lane_order_generators)):
            ...
            if (
                ...
                self._chat(
                    f"/{lane_order_generators[i].leader()['CarNumber']} {violation_message} - Passed the Leader"
                )

            for car in lane_order_generators[i].out_of_place_cars:
                self._chat(
                    f"/{car['CarNumber']} {violation_message} - Passed the {', '.join(car['IncorrectOvertakes'])} car{'s' if len(car['IncorrectOvertakes']) > 1 else ''}."
                )
