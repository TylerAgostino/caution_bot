...
chat_ollama = ChatOllama(
    base_url="http://192.168.1.125:11434", model="llama3.1:8b", temperature=0.8
)
...
def call_model(state: State):
    try:
        ...
        response = chain.invoke(
            {"messages": state["messages"], "user_prompt": state["user_prompt"]}
        )
...
def generate_caution_reason(
    llm_prompt: str = "Why was the safety car deployed at Le Mans?",
) -> str:
    try:
...
def generate_black_flag_reason(
    llm_prompt: str = "Why was the black flag shown at Le Mans?",
) -> str:
    try:
        app = workflow.compile()
        response = app.invoke(
