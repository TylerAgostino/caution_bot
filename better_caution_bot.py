...
from modules import STARTUP_LOGO, init_logging, set_logger
...
if "logger" not in st.session_state:
    ...
    for handler in st.session_state.logger.handlers:
        handler.doRollover()
    st.session_state.base_logger = logging.LoggerAdapter(
        st.session_state.logger, {"event": "base"}
    )
    st.session_state.base_logger.info(STARTUP_LOGO)
    st.session_state.base_logger.info("---")
    # Store logger in global context for BaseEvent access (used by both Streamlit and Flet)
    set_logger(st.session_state.logger, st.session_state.logfile)
...
def main():
    ...
    """
    from modules.page_sources import beer_goggles, f1_qualifying, race_control

    # List of available pages for navigation
    page_list = [race_control, beer_goggles, f1_qualifying]
    ...
    with st.sidebar:
        ...
        st.session_state.logger.handlers[0].setLevel(
        ...
        with st.container(height=450):
            ...
            log_content = "\n".join(log_lines)
            st.code(
                log_content, language="log", wrap_lines=False
            )  # Display the log content in a text area
